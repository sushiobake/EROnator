...
export default function AdminTagsPage() {
  ...
  const [historyUseRemote, setHistoryUseRemote] = useState(false);
        ...
        <section style={{ marginTop: '1rem' }}>
          ...
          {historyLoading && historyItems.length === 0 ? (
            ...
            {historyDetailRowId != null && (() => {
              ...
              const steps = Array.isArray(row?.questionHistory) ? row!.questionHistory as Array<{ qIndex?: number; kind?: string; displayText?: string; answer?: string; exploreTagKind?: string; wasNoisy?: boolean; tagCoverage?: number; confidenceBefore?: number; confidenceAfter?: number }> : [];
              return (
                <div
                  style={{
                    position: 'fixed',
                ...
                >
                  ...
                  >
                    <div style={{ padding: '1rem', borderBottom: '1px solid #eee', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <strong>過程（質問・回答の流れ）</strong>
                      <button
                        type="button"
                        onClick={() => setHistoryDetailRowId(null)}
                        style={{ padding: '0.25rem 0.75rem', background: '#666', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}
                      >
                        閉じる
                      </button>
                    </div>
                    <div style={{ overflow: 'auto', padding: '1rem', maxHeight: '70vh' }}>
                      {steps.length === 0 ? (
                        <p style={{ color: '#666' }}>記録がありません。</p>
                      ) : (
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
                          <thead>
                            <tr style={{ background: '#f0f0f0' }}>
                              ...
                              <th style={{ padding: '0.5rem', textAlign: 'left' }}>質問</th>
                              <th style={{ padding: '0.5rem', textAlign: 'center' }}>回答</th>
                              <th style={{ padding: '0.5rem', textAlign: 'center' }}>ミス</th>
                              <th style={{ padding: '0.5rem', textAlign: 'right' }}>p値</th>
                              <th style={{ padding: '0.5rem', textAlign: 'right' }}>確度</th>
                            </tr>
                          </thead>
                          <tbody>
                            {steps.map((step, i) => (
                              <tr key={i} style={{ borderBottom: '1px solid #eee' }}>
                                <td style={{ padding: '0.5rem' }}>{step.qIndex ?? i + 1}</td>
                                <td style={{ padding: '0.5rem' }}>
                                  <span style={{
                                    display: 'inline-block',
                                    padding: '0.2rem 0.5rem',
                                    background: step.kind === 'EXPLORE_TAG' ? RANK_BG.S : step.kind === 'SOFT_CONFIRM' ? RANK_BG.B : step.kind === 'HARD_CONFIRM' ? RANK_BG.X : '#e0e0e0',
                                ...
                                <td style={{ padding: '0.5rem', textAlign: 'center' }}>
                                  <span style={{
                                    color: step.answer === 'YES' ? '#2e7d32' : step.answer === 'NO' ? '#c62828' : '#666',
                                    fontWeight: 'bold',
                                  }}>
                                    {step.answer ?? '—'}
                                  </span>
                                ...
                                <td style={{ padding: '0.5rem', textAlign: 'center' }}>
                                  {step.wasNoisy != null && step.wasNoisy ? <span style={{ color: '#ff6600' }}>!</span> : '—'}
                                </td>
                                <td style={{ padding: '0.5rem', textAlign: 'right' }}>
