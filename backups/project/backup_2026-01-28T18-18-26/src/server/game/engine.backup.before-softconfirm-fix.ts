...
export async function selectNextQuestion(
  ...
  if (shouldConfirm) {
    ...
    if (confirmType === 'SOFT_CONFIRM' && derivedTags.length > 0) {
      // SOFT_CONFIRM: 案C - top1が持つレアタグを優先
      // 1. top1が持つタグを優先的に選ぶ
      // 2. その中でカバレッジが低い（レア）なものを選ぶ
      // 3. top1が持つタグがなければ、確率ベースで0.5に近いものを選ぶ
      // 
      // 狙い: top1が持つレアタグを聞くことで
      // - YESなら収束が促進される
      // - NOでも「他の作品も持っていない」ので崩壊しにくい
      ...
      // top1が持つタグを抽出
      const top1Tags = tagScores.filter(t => t.top1HasTag);
      
      let selectedTag: typeof tagScores[0] | undefined;
      
      if (top1Tags.length > 0) {
        // top1が持つタグの中で、カバレッジが低い（レア）なものを選ぶ
        // ただし、極端にレア（coverage < 0.05）は避ける（情報量が少ない）
        const usableTags = top1Tags.filter(t => t.coverage >= 0.05);
        
        if (usableTags.length > 0) {
          // カバレッジが低い順（レアな順）にソート
          usableTags.sort((a, b) => {
            if (a.coverage !== b.coverage) {
              return a.coverage - b.coverage; // 低いほど優先
            }
            return a.tag.tagKey.localeCompare(b.tag.tagKey);
          });
          selectedTag = usableTags[0];
          console.log(`[SOFT_CONFIRM] 案C: top1のレアタグ: ${selectedTag.tag.displayName} (coverage: ${(selectedTag.coverage * 100).toFixed(1)}%)`);
        } else {
          // 極端にレアしかない場合は、その中で最もcoverageが高いものを選ぶ
          top1Tags.sort((a, b) => b.coverage - a.coverage);
          selectedTag = top1Tags[0];
          console.log(`[SOFT_CONFIRM] 案C: top1のタグ(極レア): ${selectedTag.tag.displayName} (coverage: ${(selectedTag.coverage * 100).toFixed(1)}%)`);
        }
      } else {
        // top1が持つタグがない場合は、確率ベースで0.5に近いものを選ぶ
        tagScores.sort((a, b) => {
        ...
        console.log(`[SOFT_CONFIRM] 案C: フォールバック(確率ベース): ${selectedTag?.tag.displayName} (p: ${selectedTag?.p.toFixed(3)})`);
      }
