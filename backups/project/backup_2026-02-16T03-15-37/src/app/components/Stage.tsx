  ...
  useEffect(() => {
    ...
    if (isLandscape) {
      const update = () => {
        ...
        const availW = window.outerWidth - margin;
        let availH = window.outerHeight - MOBILE_FOOTER_HEIGHT - margin - MOBILE_LANDSCAPE_SAFE_HEIGHT;
    ...
    const update = () => {
      const w = window.outerWidth;
      const h = window.outerHeight - MOBILE_FOOTER_HEIGHT;
      setMobileCenterScale(getMobileCenterScale(w, Math.max(0, h)));
    };
    update();
    window.addEventListener('resize', update);
    return () => window.removeEventListener('resize', update);
        ...
        >
          <div
            style={{
              ...
              overflow: 'hidden',
              boxShadow: 'inset 0 1px 0 0 rgba(255,255,255,0.5), 0 0 0 4px #000, 0 6px 24px rgba(0,0,0,0.14)',
            }}
          >
