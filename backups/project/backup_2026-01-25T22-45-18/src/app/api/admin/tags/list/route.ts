  ...
  try {
    ...
    if (tags.length === 0) {
      try {
        ...
        // SQLクエリを動的に構築
        const questionTemplateSelect = hasQuestionTemplate ? 't.questionTemplate' : 'NULL';
        const questionTemplateGroupBy = hasQuestionTemplate ? ', t.questionTemplate' : '';
        ...
        const directTags = db.prepare(`
          SELECT 
            ...
            t.tagType,
            t.category,
            ${questionTemplateSelect} as questionTemplate,
            COUNT(wt.tagKey) as workCount
          FROM Tag t
          LEFT JOIN WorkTag wt ON t.tagKey = wt.tagKey
          GROUP BY t.tagKey, t.displayName, t.tagType, t.category${questionTemplateGroupBy}
