...
export async function POST(request: NextRequest) {
  try {
    ...
    let allWorks = await prisma.work.findMany({
      select: {
        workId: true,
        isAi: true,
      },
    ...
    if (allWorks.length === 0) {
      try {
        ...
        const directWorks = db.prepare('SELECT workId, isAi FROM Work').all() as Array<{
          workId: string;
          isAi: string;
        }>;
        ...
        const errorPayload = { error: 'Failed to load works from database' };
        return new NextResponse(JSON.stringify(errorPayload), {
          status: 500,
          headers: { 'Content-Type': 'application/json; charset=utf-8' },
        });
