...
export async function selectNextQuestion(
  ...
  if (shouldConfirm) {
      ...
      try {
        ...
        derivedTags = directTags.map(tag => {
          ...
          return {
            ...
            questionTemplate: tag.questionTemplate,
            workTags,
          };
        });
    ...
    if (confirmType === 'SOFT_CONFIRM' && derivedTags.length > 0) {
      ...
      const displayText = tag.questionTemplate
        ? tag.questionTemplate.replace(/{displayName}/g, tag.displayName)
        : `この作品は「${tag.displayName}」ですか？`;
      return {
        kind: 'SOFT_CONFIRM',
        displayText,
        tagKey: tag.tagKey,
...
async function selectExploreQuestion(
  ...
  let allTags = await prisma.tag.findMany({
    ...
    select: {
      ...
      displayName: true,
      tagType: true,
      questionTemplate: true, // 質問テンプレートを取得
    },
  });
  ...
  if (allTags.length === 0 && passingTagKeys.length > 0) {
    try {
      ...
      // eslint-disable-next-line @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access
      allTags = db.prepare(`
        SELECT tagKey, displayName, tagType, questionTemplate
        FROM Tag
  ...
  const displayText = selectedTag.questionTemplate
    ? selectedTag.questionTemplate.replace(/{displayName}/g, selectedTag.displayName)
    : `この作品は「${selectedTag.displayName}」ですか？`;
