    ...
    if (confirmType === 'SOFT_CONFIRM' && derivedTags.length > 0) {
      ...
      const tagScores = derivedTags
        ...
        .map(tag => {
          ...
          const distanceFromHalf = Math.abs(p - 0.5);
          return {
            tag,
            p,
            distanceFromHalf,
      ...
      const selectedTag = tagScores[0];
      if (selectedTag) {
        console.log(`[SOFT_CONFIRM] 確率ベース: ${selectedTag.tag.displayName} (p: ${selectedTag.p.toFixed(3)})`);
