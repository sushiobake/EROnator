...
スマホ用は PC レイアウトの縮小ではなく、**ガラリと変えて最適化する**。以下を基本方針とする。

### レイアウト

| 項目 | 方針 |
|------|------|
| **キャラ** | 少し小さくする。**位置は左下固定**（白板風の枠の内側） |
| **白板** | **大きくする**。キャラを左寄せした分、白板がメイン表示になり読みやすい |
| **スクロール** | **あり**。最低限「プレイ画面が収まる」であれば問題ない |
| **フッター** | 固定フッターにしない。問い合わせ・SNS は**スクロールで見える**位置に置く |
...
|------|------|
| **向き** | **縦持ちを基本**。横持ちは想定しない |
| **対象** | スマホ全般（タブレット含むかは要検討） |
| **優先** | **どのデバイスでも快適にプレイできる**ことを最優先にする |

### PC版との棲み分け（イメージ）

- **PC**: キャラ左・白板右の横並び。ビューポート固定。
- **スマホ**: キャラ左下固定・白板メイン。縦スクロールあり。フッターは固定しない。
