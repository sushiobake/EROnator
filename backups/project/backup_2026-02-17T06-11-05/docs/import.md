...
### OFFICIALタグ除外ルール（取得禁止タグ）

OFFICIALタグは、**config/bannedTags.json**（取得禁止タグリスト）に基づいてフィルタリングされます。DMMインポート・バッチ・importToDb のいずれもこの設定を参照します。

- **exact**: 表示名が完全一致したタグを除外
- **startsWith**: 表示名が指定文字列で始まるタグを除外
- **contains**: 表示名に指定文字列を含むタグを除外
- **regex**: 表示名が正規表現にマッチするタグを除外

一覧の追加・削除は管理画面の「取得禁止タグ管理」または `config/bannedTags.json` の編集で行います。既にDBに残っている禁止タグは `npx tsx scripts/remove-official-banned-tags.ts` で削除できます（`--dry-run` で事前確認可）。
