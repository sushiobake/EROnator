...
/**
 * OFFICIALタグをフィルタリング（取得禁止タグは config/bannedTags.json で判定）
 */
    ...
    await prisma.$transaction(async (tx) => {
      for (const workData of works) {
        ...
        try {
          ...
          // OFFICIAL（Sタグ）: 既存のみ紐付け。新規Tagは作らない（docs/s-tag-and-banned-tags.md）
          for (const displayName of keptOfficialTags) {
            const tagKey = await resolveOfficialTagKeyByDisplayName(tx, displayName);
            if (!tagKey) {
          ...
          for (const derivedTagData of derivedTagsLimited) {
            ...
            let existingTagKey = await resolveTagKeyForDisplayName(tx, derivedTagData.displayName);
            let existingTag = existingTagKey
              ? await tx.tag.findUnique({ where: { tagKey: existingTagKey } })
              : null;
