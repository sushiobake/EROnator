#!/usr/bin/env tsx
/**
 * チェック待ちの先頭12件を commentText 付きで export（精査用）
 * Usage: npx tsx scripts/export-pending-12-with-comment.ts
 * Output: data/chatgpt-export/pending-12-for-audit.json
 */
...
async function main() {
  ...
  const outPath = path.join(process.cwd(), 'data', 'chatgpt-export', 'pending-12-for-audit.json');
