#!/usr/bin/env tsx
/**
 * manual-analysis-no-derived-424works.json の分析結果をDBに反映
 * approve API と同じロジック（既存DERIVED削除→matched/suggested/characterを追加）
 */
...
async function main() {
  const jsonPath = path.join(process.cwd(), 'data', 'chatgpt-export', 'manual-analysis-no-derived-424works.json');
  if (!fs.existsSync(jsonPath)) {
    console.error('Not found:', jsonPath, '(run merge-manual-analysis-no-derived.ts first)');
    process.exit(1);
  }
