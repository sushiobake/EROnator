#!/usr/bin/env tsx
/**
 * チェック待ちの先頭100件を、レビュー用に workId / title / タグ・キャラ 付きで export する
 * Usage: npx tsx scripts/export-pending-100-for-review.ts
 * Output: data/chatgpt-export/pending-100-for-review.json
 */
...
async function main() {
  ...
  const outPath = path.join(process.cwd(), 'data', 'chatgpt-export', 'pending-100-for-review.json');
