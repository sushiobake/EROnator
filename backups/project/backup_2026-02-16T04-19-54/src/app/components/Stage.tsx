...
interface StageProps {
  ...
  /** トップ画面のみロゴを表示する */
  showLogo?: boolean;
  /** スマホのみ：中央上の横長の板に表示する内容（タイトル・質問文・番号など） */
...
function getScale(): number {
  ...
  const h = Math.max(1, window.outerHeight - PC_FOOTER_HEIGHT - 8);
...
/** スマホ：横長キャンバス、中央上に横長の板、左キャラ・右白板（正方形）。フッターはキャンバス内。 */
function MobileStageInner({
  ...
  return (
    <>
      ...
      {/* メイン：左キャラ、右白板（正方形、質問の下） */}
      <div
        style={{
          position: 'absolute',
          left: 0,
          top: topContent ? 12 + MOBILE_TOP_BOARD_HEIGHT + 8 : 0,
          right: 0,
          bottom: MOBILE_FOOTER_HEIGHT,
          display: 'flex',
          flexDirection: 'row',
          padding: topContent ? '0 12px 0 8px' : '8px',
          gap: 8,
          zIndex: 1,
        }}
      >
...
export function Stage({ children, showLogo, topContent }: StageProps) {
  ...
  if (isMobile) {
    return (
      ...
      >
        ...
        >
          <MobileStageInner showLogo={showLogo} topContent={topContent}>{children}</MobileStageInner>
        </div>
      </div>
    );
  }
