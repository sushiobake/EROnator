              ...
              await prisma.$transaction(async (tx) => {
                ...
                if (isPostgres) {
                  await tx.$executeRawUnsafe(
                    'UPDATE "Work" SET "manualTaggingFolder" = $1, "updatedAt" = NOW(), "aiAnalyzed" = true, "checkQueueAt" = $2 WHERE "workId" = $3',
                    'pending',
                    nowIso,
                    workId
                  );
                  ...
                  await tx.$executeRawUnsafe(
                    'UPDATE Work SET manualTaggingFolder = ?, updatedAt = datetime(\'now\'), aiAnalyzed = 1, checkQueueAt = ? WHERE workId = ?',
                    'pending',
                    nowIso,
                    workId
                  );
