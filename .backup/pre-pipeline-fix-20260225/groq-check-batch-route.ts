...
上記の works 配列の各作品をチェックし、results 配列（works と同じ順序・件数）で返せ。`;
                ...
                try {
                  ...
                  throw new Error(`Phase1 失敗 (${chunk.map((w) => w.workId).join(',')}): ${msg}`);
                }

                const jsonMatch1 = content1.match(/\{[\s\S]*\}/);
                if (!jsonMatch1) throw new Error('No JSON in response');
                const parsed1Batch = JSON.parse(jsonMatch1[0]) as Phase1Batch;
...
上記の works 配列の各作品について、API1 が指摘した api1Issues に従い、不足タグの追加提案を出し、results 配列（works と同じ順序・件数）で返せ。`;
                  ...
                  if (content2) {
                    ...
                    try {
                      ...
                      if (jsonMatch2) {
                        const parsed2Batch = JSON.parse(jsonMatch2[0]) as Phase2Batch;
                ...
                return { chunkStart, phase1Items };
              })
            )
          );

          for (const { phase1Items } of chunkResults.sort((a, b) => a.chunkStart - b.chunkStart)) {
