...
上記の works 配列の各作品にタグを付け、results 配列（works と同じ順序・件数）で返せ。`;
                ...
                try {
                  ...
                  throw new Error(`Phase0 失敗 (${chunk.map((w) => w.workId).join(',')}): ${msg}`);
                }

                const jsonMatch = content.match(/\{[\s\S]*\}/);
                if (!jsonMatch) throw new Error('No JSON in response');
                const parsed = JSON.parse(jsonMatch[0]) as TagBatch;
                ...
                return { chunkStart, chunk, parsed };
              })
            )
          );
          ...
          for (const { chunk, parsed } of chunkResults.sort((a, b) => a.chunkStart - b.chunkStart)) {
