...
export default function TagManager({ adminToken }: Props) {
  ...
  const [specialQuestions, setSpecialQuestions] = useState<{
    ...
    TITLE_CHAR_TYPE?: { KANJI?: string; KATAKANA?: string; HIRAGANA?: string };
  ...
  const [editingSpecial, setEditingSpecial] = useState<{ type: string; key?: string; id?: string } | null>(null);
  ...
  const handleSaveSpecialQuestion = async () => {
    ...
    try {
      ...
      if (editingSpecial.id && editingSpecial.type === 'TITLE_SYLLABLE') body.key = editingSpecial.id;
      if (editingSpecial.type === 'POPULARITY' && editingSpecial.key === 'popularityThreshold') {
        ...
        <div style={{ padding: '15px', backgroundColor: '#f0f8ff', borderRadius: '0 0 8px 8px' }}>
          {specialQuestions && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              {/* TITLE_CHAR_TYPE */}
              <div>
                <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>タイトル文字種</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  {(['KANJI', 'KATAKANA', 'HIRAGANA'] as const).map(k => (
                    <div key={k} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <span style={{ width: '80px' }}>{k === 'KANJI' ? '漢字' : k === 'KATAKANA' ? 'カタカナ' : 'ひらがな'}:</span>
              ...
              {/* TITLE_SYLLABLE */}
              <div>
                <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>50音分類</div>
                {((specialQuestions.TITLE_SYLLABLE?.ranges ?? []).length ? specialQuestions.TITLE_SYLLABLE!.ranges! : []).map((r, i) => (
                  <div key={r.id} style={{ marginBottom: '8px' }}>
                    <div style={{ fontSize: '12px', color: '#666', marginBottom: '2px' }}>{r.label}</div>
                    {editingSpecial?.type === 'TITLE_SYLLABLE' && editingSpecial.id === r.id ? (
                      <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                        <input
                          type="text"
                          value={editingSpecialValue}
