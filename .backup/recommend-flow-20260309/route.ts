...
/** 最終優先: 重要無名=3, 重要有名=2.5, 無名=1.5, 有名=1 */
const WEIGHT_IMPORTANT_UNKNOWN = 3;
const WEIGHT_IMPORTANT_FAMOUS = 2.5;
const WEIGHT_UNKNOWN = 1.5;
const WEIGHT_FAMOUS = 1;
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 旧形式: tagKeys のみ → 全タグを「有名」として扱いスコア
    ...
    const useNewScoring = selectedFamous.length > 0 || selectedUnknown.length > 0;
    ...
    const scored = works.map(w => {
      ...
      if (useNewScoring) {
        ...
        for (const wt of w.workTags) {
          ...
          if (importantUnknownSet.has(k)) sum += WEIGHT_IMPORTANT_UNKNOWN;
          else if (importantFamousSet.has(k)) sum += WEIGHT_IMPORTANT_FAMOUS;
          else if (unknownSet.has(k)) sum += WEIGHT_UNKNOWN;
          else if (famousSet.has(k)) sum += WEIGHT_FAMOUS;
        }
      ...
      const matchedTagCount = useNewScoring
        ? w.workTags.filter(wt => {
            const k = normalizeTagKey(wt.tagKey);
            return famousSet.has(k) || unknownSet.has(k);
          }).length
        : w.workTags.filter(wt => legacyTagKeys.includes(wt.tagKey)).length;
      const totalSelected = useNewScoring ? famousSet.size + unknownSet.size : legacyTagKeys.length;
    ...
    if (wantDebug) {
      ...
      for (const t of selectedFamous) {
        const w = t.important ? WEIGHT_IMPORTANT_FAMOUS : WEIGHT_FAMOUS;
      ...
      for (const t of selectedUnknown) {
        const w = t.important ? WEIGHT_IMPORTANT_UNKNOWN : WEIGHT_UNKNOWN;
        tagsWithWeights.push({
          tagKey: t.tagKey,
          displayName: tagKeyToDisplayName.get(t.tagKey) ?? t.tagKey,
          weight: w,
