...
■ ゴール（ここまでやって終わり）
・あなたがタグ付け（通常タグ＋キャラタグ）→ JSON を返す → その JSON を保存 → apply を実行 → 対象作品が「チェック待ち」フォルダの先頭に並ぶ。
・apply を実行しないとチェック待ちに並ばない。必ず apply まで実行すること。

■ あなたがやること（順番に実行）
1. 以下の workIds を引数に、DB から作品データを取得: npx tsx scripts/fetch-works-for-ai.ts <workIds（スペース区切り）>
2. 取得した JSON の workId, title, commentText を使ってタグ付けする。docs/legacy-ai-tagging-instruction.md に従う。各作品: (1) タイトルからタグを取る (2) **キャラタグ**: commentText に登場人物の名前が出ていれば characterName に必ず入れる、なければ null (3) タイトルに数字・続編・総集編・編があればシリーズ系タグを1つ。
3. 出力: workId, title, matchedTags, suggestedTags, additionalSTags, characterName, tagReasoning の JSON 配列。workId は取得した値そのまま。
4. 返した JSON を data/chatgpt-export/ に保存（例: cursor-analysis-untagged-10-batch1.json）
5. npx tsx scripts/apply-cursor-legacy-ai-batch.ts 保存したファイル名.json を実行
→ 完了。

【対象 workIds】
`;
      const workIdList = batchWorks.map((w) => w.workId).join('\n');
      const text = instruction + workIdList;
  ...
  const copyForCheck = useCallback(async () => {
    ...
    try {
      const instruction = `【AIへの指示】

■ ゴール
・タグチェックを行い、各作品について「タイトル＋コメント」と「公式タグ＋付け加えたタグ」の対応を確認する。問題なければ「タグ済」、問題あれば「人間による確認が必要」と判定して返す。

■ あなたがやること（順番に実行）
1. 以下の workIds を引数に、DB から作品データを取得: npx tsx scripts/fetch-works-for-ai.ts --check <workIds（スペース区切り）>
2. 取得した JSON（workId, title, commentText, derivedTags, officialTags, characterName）を使ってチェック。docs/check-instruction.md に従う。
3. 全作品に checkReasoning（タイトル照合・各タグ根拠・キャラ）を必須で含めること。タグを修正した場合は tagChanges（added/removed）を明記。
4. 返した JSON 全文を会話に貼り付けること。
