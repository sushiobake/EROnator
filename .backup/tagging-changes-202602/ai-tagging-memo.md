...
## 新規提案タグが多くなった場合の検討事項（将来用メモ・2026-02）

チェックGPTの「新規提案」（既存S/A/Bにないタグの提案）が多くて運用が困った場合、以下を取り入れる価値がある。可能性は大いにある。

- **新規提案のための重複チェック** … 類似・同義のタグが既にないか事前に検証する
- **類似タグ検索** … 提案された語と既存タグの類似度を調べ、重複を避ける
- **追加提案と新規提案をまとめて扱う特殊フロー** … 一括処理や優先度付き反映
- **エラー時のリトライやロールバックの込み入った制御** … 部分失敗時の復旧・再試行
