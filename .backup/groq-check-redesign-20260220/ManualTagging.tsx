      ...
      <div style={{ marginBottom: '1rem', padding: '0.75rem', border: '1px solid #ccc', borderRadius: '6px', background: '#f9f9f9' }}>
        ...
        {filter === 'pending' && (
          <>
            <p style={{ color: '#555', fontSize: '0.8rem', marginBottom: '0.5rem' }}>
              ...
              {' '}または「Groqで1件チェック」でチェック待ちの先頭1件を自動チェックできます。
            </p>
          ...
          {filter === 'pending' && (
            <button
              ...
              onClick={async () => {
                ...
                try {
                  const res = await fetch('/api/admin/groq-check-one', { method: 'POST' });
                  const data = await res.json();
                  if (data.success) {
                    alert(`✅ ${data.workId} をチェックしました（${data.result}）`);
                    fetchList();
                    fetchCounts();
            ...
            >
              {groqCheckLoading ? 'Groqチェック中…' : 'Groqで1件チェック'}
            </button>
          )}
        </div>
        ...
        {(filter === 'pending' || filter === 'legacy_ai') && (
          <details style={{ marginTop: '0.5rem', fontSize: '0.85rem' }}>
            ...
            <ol style={{ margin: '0.5rem 0 0 1.2rem', padding: 0, lineHeight: 1.6 }}>
              <li>AIが返した<strong>チェック結果のJSON</strong>（各要素に workId, result: 「タグ済」or「人間による確認が必要」）を、<code style={{ background: '#eee', padding: '0.1rem 0.3rem' }}>data/chatgpt-export/check-result.json</code> に保存する</li>
                ...
                {detail.lastCheckTagChanges &&
                  ...
                  <div style={{ padding: '0.6rem', background: '#f0f8ff', border: '1px solid #b8d4e8', borderRadius: '6px', marginBottom: '0.5rem' }}>
                    ...
                    <div style={{ fontSize: '0.75rem', color: '#555', marginBottom: '0.4rem' }}>
                      追加・削除・新規提案を個別に受け入れてください。受け入れたあと保存で確定します。
                    </div>
                    {detail.lastCheckTagChanges.added.length > 0 && (
                            ...
                            <span key={tag} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.2rem' }}>
                              ...
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  if (rank === 'S') setFormAdditionalS((p) => (p.includes(tag) ? p : [...p, tag]));
                                  else if (rank === 'A') setFormA((p) => (p.includes(tag) ? p : [...p, tag]));
                                  else if (rank === 'B') setFormB((p) => (p.includes(tag) ? p : [...p, tag]));
                                  else setFormC((p) => (p.includes(tag) ? p : [...p, tag]));
                                  setDetail((d) =>
                                    d?.lastCheckTagChanges
                                      ? { ...d, lastCheckTagChanges: { ...d.lastCheckTagChanges, added: d.lastCheckTagChanges.added.filter((x) => x !== tag) } }
                                      : d
                                  );
                                  setDirty(true);
                                }}
                            ...
                            <span key={tag} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.2rem' }}>
                              ...
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  setFormAdditionalS((p) => p.filter((x) => !remSet.has(x)));
                                  setFormA((p) => p.filter((x) => !remSet.has(x)));
                                  setFormB((p) => p.filter((x) => !remSet.has(x)));
                                  setFormC((p) => p.filter((x) => !remSet.has(x)));
                                  setDetail((d) =>
                                    d?.lastCheckTagChanges
                                      ? { ...d, lastCheckTagChanges: { ...d.lastCheckTagChanges, removed: d.lastCheckTagChanges.removed.filter((x) => x !== tag) } }
                                      : d
                                  );
                                  setDirty(true);
                                }}
                          ...
                          {(['A', 'B', 'C'] as const).map((rank) => (
                            <button
                              ...
                              onClick={() => {
                                ...
                                if (rank === 'A') setFormA((p) => (p.includes(tag) ? p : [...p, tag]));
                                else if (rank === 'B') setFormB((p) => (p.includes(tag) ? p : [...p, tag]));
                                else setFormC((p) => (p.includes(tag) ? p : [...p, tag]));
                                setDetail((d) =>
