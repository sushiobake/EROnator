 ...
 * result が「タグ済」→ tagged、「人間による確認が必要」→ has_issues（問題あり）。Phase2で提案すると needs_human_check（人間確認）へ
...
function toFolder(result: string): 'tagged' | 'has_issues' | null {
  const r = (result || '').trim();
  if (r === 'タグ済') return 'tagged';
  if (r === '人間による確認が必要') return 'has_issues'; // Phase1→問題あり。Phase2で提案すると人間確認(needs_human_check)へ
  return null;
}
...
async function main() {
  ...
  for (let i = 0; i < listToProcess.length; i++) {
    ...
    // has_issues（Phase1）: removed のみ。needs_human_check（Phase2）: added/newProposal あり
    const tagChangesPayload: { added: string[]; removed: string[]; newProposal?: string } =
      folder === 'has_issues'
        ? {
            added: [],
            removed: Array.isArray(item.tagChanges?.removed) ? item.tagChanges.removed : [],
          }
        : folder === 'needs_human_check'
          ? {
              added: Array.isArray(item.tagChanges?.added) ? item.tagChanges.added : [],
              removed: Array.isArray(item.tagChanges?.removed) ? item.tagChanges.removed : [],
              ...(item.tagSuggestions?.newProposal && item.tagSuggestions.newProposal.trim()
                ? { newProposal: item.tagSuggestions.newProposal.trim() }
                : {}),
            }
          : { added: [], removed: [] };
    const tagChangesJson = folder === 'has_issues' || folder === 'needs_human_check' ? JSON.stringify(tagChangesPayload) : null;

    // tagged / has_issues / needs_human_check はゲーム有効（gameRegistered=true, needsReview=false）
    if (folder === 'tagged') {
      ...
      // has_issues または needs_human_check
      if (isPostgres) {
        await prisma.$executeRawUnsafe(
