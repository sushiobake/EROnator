...
import { loadSpecialQuestionsConfig } from '@/server/config/specialQuestionsLoader';
...
export interface SpecialQuestionResult {
  ...
  /** Type TITLE_SYLLABLE: 対象文字（titleReadingInitial が含まれるか） */
...
export async function selectSpecialQuestion(
  ...
  if ((!allowedTypes || allowedTypes.includes('TITLE_SYLLABLE')) && !usedSpecialTypes.has('TITLE_SYLLABLE')) {
    ...
    if (syllableOptions.length > 0) {
      ...
      for (const opt of syllableOptions) {
        ...
        let pYes = 0;
        for (const p of probabilities) {
          const initial = workInitialMap.get(p.workId);
          if (initial && charSet.has(initial)) {
            pYes += p.probability;
          }
        }
  ...
  // スロット3,5,9,11,16: 情報量上位2〜3件からランダム選択（A3）
  if (slotIndex === 3 || slotIndex === 5 || slotIndex === 9 || slotIndex === 11 || slotIndex === 16) {
