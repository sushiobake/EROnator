...
export interface QuestionData {
  ...
  /** SPECIAL_QUESTION TITLE_SYLLABLE の対象文字（titleReadingInitial が含まれるか） */
...
export async function selectNextQuestion(
  ...
  const baseSpecialSlots = config.flow.specialQuestionSlotIndices ?? [3, 5, 9];
  const hasSpecialAnsweredUnknown = questionHistory.some(
    q => q.kind === 'SPECIAL_QUESTION' && q.answer === 'UNKNOWN'
  );
  const specialSlotIndices = hasSpecialAnsweredUnknown && !baseSpecialSlots.includes(11)
    ? [...baseSpecialSlots, 11]
    : baseSpecialSlots;
  if (specialSlotIndices.includes(qIndex)) {
    ...
    const workIds = weights.map(w => w.workId);
    const specialResult = await selectSpecialQuestion(probabilities, usedSpecialTypes, workIds, qIndex, titleCharTypeAnsweredUnknown);
    if (specialResult) {
      return {
...
export async function processAnswer(
  ...
  if (question.kind === 'SPECIAL_QUESTION' && question.specialQuestionType === 'TITLE_SYLLABLE') {
    ...
    const workHasFeature = (workId: string): boolean => {
      const w = works.find(x => x.workId === workId);
      if (!w?.titleReadingInitial) return false;
      return charSet.has(w.titleReadingInitial);
    };
    const useBayesian = config.algo.useBayesianUpdate !== false;
    if (useBayesian) {
      const epsilon = config.algo.bayesianEpsilon ?? 0.02;
