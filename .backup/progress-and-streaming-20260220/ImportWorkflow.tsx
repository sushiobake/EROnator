...
export default function ImportWorkflow() {
  ...
  const handleApiImport = async () => {
    ...
    setApiLoading(true);
    setApiResult(null);
    setProgress('import', { current: 0, total: Math.min(apiCount, 100) * Math.max(1, Math.min(20, apiRounds)), phase: 'APIインポート中' });
    try {
  ...
  const handleFetchSelectedComments = async () => {
    ...
    setProgress('import', { current: 0, total: selectedWorkIds.size, phase: 'コメント取得中' });
    try {
  ...
  const handleApiThenCommentBulk = async () => {
    ...
    setProgress('import', { current: 0, total: 2, phase: 'API取得中' });
    ...
    try {
      ...
      fetchStats();
      setProgress('import', { current: 1, total: 2, phase: 'コメント取得中' });
