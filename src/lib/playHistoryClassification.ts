...
/**
 * プレイの questionHistory から最大確信度（confidenceAfter の最大値）を返す。
 * 「あと一歩で当てられたか」の判定に使う。値が取れなければ null。
 */
export function computeMaxConfidence(row: { questionHistory?: unknown }): number | null {
  if (!Array.isArray(row.questionHistory)) return null;
  let max = -Infinity;
  for (const raw of row.questionHistory as Array<{ confidenceAfter?: number }>) {
    const v = typeof raw?.confidenceAfter === 'number' ? raw.confidenceAfter : null;
...
/**
 * 訪問者の総プレイ数から、管理画面の「訪問者」列に出すバッジ情報を返す。
 * - null / 1 回 : 「🆕 初回」
 * - 2 回以上 : 「🔁 ×N」
 */
export interface VisitorBadge {
  ...
  color: string;
  bg: string;
  tone: 'new' | 'repeat' | 'unknown';
}

export function resolveVisitorBadge(count: number | null | undefined): VisitorBadge {
  if (count == null) {
    return { label: 'ID未記録', shortLabel: '未記録', color: '#9ca3af', bg: '#f3f4f6', tone: 'unknown' };
  }
  if (count <= 1) {
    return { label: '🆕 初回', shortLabel: '初回', color: '#1d4ed8', bg: '#dbeafe', tone: 'new' };
  }
  return { label: `🔁 ×${count}`, shortLabel: `×${count}`, color: '#7c3aed', bg: '#ede9fe', tone: 'repeat' };
}

/**
 * セッション開始時刻〜終了時刻（createdAt）から滞在秒数を計算。
 * 取得できなければ null。
 */
