...
import fs from 'fs';
import { cpus } from 'os';
import { runSimulationInWorker, type WorkerResultItem } from '@/server/simulation/simulationWorkerRunner';
    ...
    const failureAnalysis = failures.length > 0 ? (() => {
      ...
      return {
        ...
        avgWasNoisyCount: withAnalysis.length > 0
          ? Math.round((withAnalysis.reduce((s, f) => s + ((f.analysisData as SimulationAnalysisData).wasNoisyCount), 0) / withAnalysis.length) * 100) / 100
          : null,
        avgCorrectRank: withDiag.length > 0
          ? Math.round((withDiag.reduce((s, f) => s + ((f.diagnostic as SimulationDiagnostic).correctRank), 0) / withDiag.length) * 100) / 100
          : null,
        avgTop1Confidence: withDiag.length > 0
          ? Math.round((withDiag.reduce((s, f) => s + ((f.diagnostic as SimulationDiagnostic).top1Confidence), 0) / withDiag.length) * 10000) / 10000
          : null,
      };
    })() : null;
...
async function runSimulation(
  ...
  try {
    ...
    while (true) {
      ...
      const earlyFailSimLocal = evaluateSimulationEarlyExitAfterQuizAnswer({
        newConfidence,
        revealThreshold,
        revealMissCount,
        maxRevealMisses: config.flow.maxRevealMisses as number,
      ...
      if (earlyFailSimLocal.stop) {
        endedBy = earlyFailSimLocal.endedBy;
        outcome = 'FAIL_LIST';
        finalWorkId = earlyFailSimLocal.finalWorkId;
        break;
      }
    }
