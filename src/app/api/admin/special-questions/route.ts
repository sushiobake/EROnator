...
export async function POST(request: NextRequest) {
  ...
  try {
    const body = (await request.json()) as Partial<SpecialQuestionsConfig> & {
      type?: 'SERIES' | 'TITLE_CHAR_TYPE' | 'POPULARITY' | 'TITLE_SYLLABLE' | 'TITLE_SYLLABLE_2' | 'AUTHOR_CHAR_TYPE';
      key?: string;
      value?: unknown;
    };
    const config = await loadSpecial();
    ...
    // TITLE_SYLLABLE_2: branches 全体 or 特定 branch の questionText / chars
    if (body.type === 'TITLE_SYLLABLE_2') {
      if (!config.TITLE_SYLLABLE_2) config.TITLE_SYLLABLE_2 = { branches: {} };
      if (body.key === 'branches' && body.value && typeof body.value === 'object') {
        config.TITLE_SYLLABLE_2.branches = body.value as NonNullable<SpecialQuestionsConfig['TITLE_SYLLABLE_2']>['branches'];
      } else if (body.key) {
        const [parentId, branch, field] = (body.key as string).split('.');
        const branches = config.TITLE_SYLLABLE_2.branches ?? {};
        if (!branches[parentId]) branches[parentId] = {};
        const parent = branches[parentId]!;
        if (branch === 'yesBranch' || branch === 'noBranch') {
          const b = branch === 'yesBranch' ? (parent.yesBranch ??= { label: '', questionText: '', chars: [] }) : (parent.noBranch ??= { label: '', questionText: '', chars: [] });
          if (field === 'questionText' && typeof body.value === 'string') {
            b.questionText = body.value.trim();
          } else if (field === 'chars') {
            b.chars = Array.isArray(body.value) ? body.value : (typeof body.value === 'string' ? body.value.split(/[,，\s]+/).map((s: string) => s.trim()).filter(Boolean) : []);
          } else if (field === 'label' && typeof body.value === 'string') {
            b.label = body.value.trim();
          }
        }
      }
    }

    // AUTHOR_CHAR_TYPE: HIRAGANA_OR_KATAKANA | KANJI_OR_ALPHA（2択、情報量最大）
    if (body.type === 'AUTHOR_CHAR_TYPE' && body.key && typeof body.value === 'string') {
      if (!config.AUTHOR_CHAR_TYPE) config.AUTHOR_CHAR_TYPE = {};
      (config.AUTHOR_CHAR_TYPE as Record<string, string>)[body.key] = body.value.trim();
    }
