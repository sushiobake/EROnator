...
import {
  readTrafficInternalFromStorage,
  clearTrafficInternalFromStorage,
  readTrafficShortCodeFromStorage,
  readTrafficCampaignTagFromStorage,
  clearTrafficShortParamsFromStorage,
} from './components/TrafficAttributionTracker';
...
/** /api/start 用: ランディング・リファラー・UTM・短縮パラメータ r/ct（サーバで JSON 化して保存） */
function collectTrafficAttributionForStart(): {
  ...
  try {
    ...
    const rFromUrl = sp.get("r")?.trim().slice(0, 32) ?? "";
    const rStored = readTrafficShortCodeFromStorage()?.trim().slice(0, 32) ?? "";
    const r = rFromUrl || rStored || undefined;
    const ctFromUrl = sp.get("ct")?.trim().slice(0, 128) ?? "";
    const ctStored = readTrafficCampaignTagFromStorage()?.trim().slice(0, 128) ?? "";
    const ct = ctFromUrl || ctStored || undefined;
    const out: {
      landing: string;
      referrer: string | null;
      utm?: Record<string, string>;
...
interface HomeClientProps {
  initialRecentSuccesses?: RecentSuccessItem[];
}

export default function HomeClient({ initialRecentSuccesses = [] }: HomeClientProps) {
  ...
  if (state === 'TOP') {
    return (
      <>
        <TopScreen topLines={gameCopy?.topLines} onPlay={handleTopPlay} onRecommend={() => setState('RECOMMEND')} streamerMode={streamerMode} recentSuccesses={initialRecentSuccesses} onToggleStreamerMode={() => {
  ...
  if (state === 'QUIZ' && question) {
    const showSyllableSupplement =
      question.kind === 'SPECIAL_QUESTION' &&
      (question.specialQuestionType === 'TITLE_SYLLABLE' ||
        question.specialQuestionType === 'TITLE_SYLLABLE_2');
    return (
      <>
        {debugUIEnabled && debugEnabled && (
          <DebugPanel
            ...
            isThinking
              ...
              : (
                <div style={isMobile ? { fontSize: 24, lineHeight: 1.3, textAlign: 'center' } : {}}>
                  ...
                  {showSyllableSupplement && (
                    <p
                      style={{
                        margin: isMobile ? '4px 0 0 0' : '3px 0 0 0',
                        color: '#dc2626',
              ...
              : isMobile
                ...
                <div>
                  ...
                  <p style={{ margin: '6px 0 0 0', color: 'var(--color-text-muted)', fontSize: 15 }}>
                    {(() => {
                      const rawText = gc?.failListSubPc ?? 'この中に近いものはある？　ないなら右で検索してみて。';
                      const bolded = 'ないなら右で検索してみて';
                      const idx = rawText.indexOf(bolded);
                      if (idx === -1) return rawText;
                      return (
                        <>
                          {rawText.slice(0, idx)}
                          <strong style={{ color: '#b45309', fontWeight: 800 }}>{bolded}</strong>
                          {rawText.slice(idx + bolded.length)}
                        </>
                      );
                    })()}
                  </p>
