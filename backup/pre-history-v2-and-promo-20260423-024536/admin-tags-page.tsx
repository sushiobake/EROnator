...
export default function AdminTagsPage() {
  ...
  const handleHistoryExportSelected = () => {
    if (historySelectedIds.size === 0) {
      alert('保存する履歴を選択してください。');
      return;
    }
    const items = historyItems.filter((r) => historySelectedIds.has(r.id));
    if (items.length === 0) {
      alert('表示中のページに選択された行がありません。再読み込み後に選び直してください。');
      return;
    }
    const stamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
    downloadAdminHistoryJson(`eronator-play-history-export-${stamp}.json`, {
      exportedAt: new Date().toISOString(),
      kind: 'play' as const,
      source: historyUseRemote ? 'remote' : 'local',
      remoteBaseUrl: historyUseRemote ? remoteDeploymentUrl || null : null,
      historyOutcome: historyOutcome || null,
      page: historyPage,
  ...
  const handleRecHistExportSelected = () => {
    if (recHistSelectedIds.size === 0) {
      alert('保存する履歴を選択してください。');
      return;
    }
    const items = recHistItems.filter((r) => recHistSelectedIds.has(r.id));
    if (items.length === 0) {
      alert('表示中のページに選択された行がありません。再読み込み後に選び直してください。');
      return;
    }
    const stamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
    downloadAdminHistoryJson(`eronator-recommend-play-history-export-${stamp}.json`, {
      exportedAt: new Date().toISOString(),
          ...
          {simResult && (
            <div style={{ 
              ...
              <div style={{ marginBottom: '0.5rem', fontSize: '0.9rem' }}>
                <span>{(simResult as { success?: boolean }).success ? '成功' : '失敗'} | <strong>作品:</strong> {simResult.targetWorkTitle} | <strong>最終:</strong> {simResult.finalWorkTitle || '(なし)'} | <strong>結果:</strong> {simResult.outcome}
        ...
        <section style={{ marginTop: '1rem' }}>
          ...
          {historyLoading && historyItems.length === 0 ? (
            ...
            <div style={{ marginBottom: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
              <button
                type="button"
                onClick={handleHistoryExportSelected}
                disabled={historySelectedIds.size === 0}
                style={{
                  padding: '0.4rem 0.75rem',
                  backgroundColor: historySelectedIds.size === 0 ? '#ccc' : '#1565c0',
                  color: 'white',
                  border: 'none',
                  borderRadius: '4px',
                ...
                <tbody>
                  ...
                  {historyFilteredItems.map(({ row, badge }) => (
                    <tr key={row.id} style={{ borderBottom: '1px solid #eee' }}>
                      ...
                      {(() => {
                          const _sns = trafficAttributionSnsColumnLabel(row.trafficAttributionJson);
                          return (
                      <td style={{ padding: '0.5rem', textAlign: 'center', fontSize: '0.85rem' }} title={_sns.title || undefined}>
                        {_sns.cell === 'ー' ? <span style={{ color: '#999' }}>ー</span> : _sns.cell}
                      </td>
                          );
                        })()}
                      <td style={{ padding: '0.5rem', textAlign: 'center' }}>
          ...
          {recHistLoading && recHistItems.length === 0 ? (
            ...
            <>
              <div style={{ marginBottom: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
                <button
                  type="button"
                  onClick={handleRecHistExportSelected}
                  disabled={recHistSelectedIds.size === 0}
                  style={{
                    padding: '0.4rem 0.75rem',
                    backgroundColor: recHistSelectedIds.size === 0 ? '#ccc' : '#1565c0',
                    color: 'white',
                    border: 'none',
                    borderRadius: '4px',
                ...
                >
                  {`選択をJSONで保存（${recHistSelectedIds.size}件）`}
                </button>
                <button
                  type="button"
