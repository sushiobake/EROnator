...
export function getEarlyExitStepSnapshot(
  ...
  const matchFlatDelta5 = confidenceDelta5 <= threshold.maxConfidenceDelta5;
  const matchedCount =
    (matchLowConfidence ? 1 : 0) + (matchWideCandidates ? 1 : 0) + (matchFlatDelta5 ? 1 : 0);
  const wouldEarlyExit = matchedCount >= requiredConditions;
  return {
    questionCountAfterAnswer: newQuestionCount,
    confidence,
    effectiveCandidates,
    ...
    thresholds: {
      ...
      maxConfidenceDelta5: threshold.maxConfidenceDelta5,
    },
    requiredConditions,
    matchLowConfidence,
    matchWideCandidates,
