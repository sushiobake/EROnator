...
export function SimEarlyExitThresholdsSummary({ flow }: { flow?: { earlyExitReview?: unknown } }) {
  ...
  const line3 = qs
    .map((q) => {
      ...
      const n = q.slice(1);
      return `${n}問:①${(t.minConfidence * 100).toFixed(0)}%②${t.maxEffectiveCandidates}③${t.maxConfidenceDelta5}`;
    })
    .join('　｜　');
  ...
  return (
    <div style={wrap} title="設定タブの flow.earlyExitReview と同じ（未保存の編集はシミュに反映されないことがあります）">
      <div style={{ fontWeight: 600 }}>
        早期失敗（現在のコンフィグ）　{ee.reviewIndices.join('・')}問直後に審査、3条件中{ee.requiredConditions}で発動
      </div>
      <div style={row}>
        ①<strong>確度</strong>（→の右）が下限未満　②<strong>候補</strong>が上限超　③<strong>直近５問</strong>が上限以下（偏りの近似・動き乏しい）
      </div>
