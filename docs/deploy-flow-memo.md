...
**次回デプロイ予定**: （sync 高速化は 2026-02 に実施済み: Tag/Work バッチ化・WorkTag 1000 件/バッチ）
...
| 順 | やること | コマンドなど |
|----|----------|--------------|
| 1 | バックアップ（任意） | `npm run backup:project` |
| 2 | 変更をコミット | `git add .` → `git commit -m "メッセージ"` |
| 3 | プレビューへデプロイ | `npm run deploy:preview` |
| 4 | Vercel の develop URL で確認 | ブラウザで「セッション開始」まで試す |

`deploy:preview` は **ローカルを変えません**（`deploy-safe-rules.md` 参照）。schema の切り替えと復元は自動。

### 従来フロー（prepare:push を手動で使う場合）
...
| # | 対策 | 実施内容 |
|---|------|----------|
| 1 | Tag / Work のバッチ化 | 1件ずつ → 100件ずつ `$transaction` でまとめる |
| 2 | WorkTag のバッチサイズ増加 | 500 → 1000 に変更 |
...
| # | 対策 | 現実性 | 一言 |
|---|------|--------|------|
| 1 | Tag/Work バッチ化 | ◎ | 実施済み（100件/バッチ） |
| 2 | バッチサイズ増加 | ◎ | 実施済み（WorkTag 1000件/バッチ） |
