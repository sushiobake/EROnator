...
**現在表示されているURLについて**:
- `https://eronator-kq2rmbj7q-obakesushi2-4362s-projects.vercel.app/` は**プレビューデプロイメントURL**です
- これは各デプロイごとに生成される一時的なURLです
- **メインURL**（固定URL）を確認する必要があります

**メインURLの確認方法**:

1. Vercelダッシュボード → プロジェクトを選択
2. プロジェクトのトップページ（**Overview**）を開く
3. 上部に表示されているURLを確認
   - 形式: `https://プロジェクト名.vercel.app`
   - 例: `https://eronator.vercel.app`
   - これが**メインURL**（固定URL）です
...
3. **Project Name** を確認・変更
   - 現在のプロジェクト名に個人名が含まれている場合は、審査用の名前に変更
   - 例: `eronator` や `eronator-mvp0` など、個人名を含まない名前に変更
4. **Save** をクリック
5. 新しいメインURLが生成されます（例: `https://eronator.vercel.app`）

**注意**: 
- **メインURL**（`プロジェクト名.vercel.app`）をFANZA審査に提出してください
- プレビューURL（`プロジェクト名-ランダム文字列-ユーザー名-projects.vercel.app`）は提出不要です
- プロジェクト名を変更すると、メインURLが変わります
- プロジェクト名は小文字のみ、ハイフンやアンダースコアが使用可能
...
1. **URLの確認**
   ...
   - [ ] **メインURLは固定されています**（デプロイするたびに変わりません）
     - 形式: `https://プロジェクト名.vercel.app`
     - 例: `https://eronator.vercel.app` または `https://eronator-mvp0.vercel.app`
     - **このURLは常に最新のデプロイを指します**（デプロイするたびに自動的に最新版に更新されます）
   - [ ] HTTPSが有効になっている（Vercelは自動で有効化）
   - [ ] **このURLをFANZA審査に提出します**（独自ドメインは審査通過後に設定可能）
   
   **注意**: 
   - メインURL（`プロジェクト名.vercel.app`）は固定で、デプロイごとに変わりません
   - 各デプロイには個別のプレビューURLも生成されますが、メインURLは常に最新のデプロイを指します
   - プロジェクト名を変更しない限り、メインURLは変わりません
